...
%% #pc_header : store header in model, interprete unit--------------------2
 ...
 if isempty(st2); 
  sdtw('_nb','Model unit interpretation failed, assuming SI');
  mdl.unit='SI';
 else
  st2=st2{1};
  if strcmpi(st2{1},'MM')&&strcmpi(st2{2},'KG')
   ...
   warning('Need to deal with unit system (%s,%s), for now SI has been assigned',st2{1},st2{2});
   mdl.unit='SI';
  end
 end
