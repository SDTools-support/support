...
wire=ufread('cc_test_wire.unv');
wire=feutil('rmfield',wire,'pl','il','header','Stack');% remove unused fields

% Read responses which will give sensor direction information
UFS=ufread('cc_test.uff');Test=UFS(1);

wire.tdof=Test.dof(:,1); % define sensor dirs
% Here there is a reorientation needed. See details in 
% http://www.sdtools.com/helpcur/ccdemo.html#cccor
wire.tdof=fe_sens('tdof',wire.tdof);
wire.tdof(:,3:5)=wire.tdof(:,3:5)*[0,0,-1;-1,0,0;0,1,0];
...
cf.mdl=fe_case(cf.mdl,'sensdof','Test',wire);
...
dr=struct('def',sens.cta*def.def,'tdof',sens.tdof, ...
    'data',def.data(:,1));
cg=feplot(5);cg.model=wire; cg.sel='-test'; cg.def=dr;fecom(cg,'colordata evaly')

%% Now compute full and partial MAC
%idenfity modes ----------------------------------------------
ci=iicom('curveinit-reset','Test',Test);
...
ID=fe_def('subdof',ci.Stack{'IdMain'},fe_c(wire.tdof,.03,'ind'));
dr=struct('def',sens.cta*def.def,'DOF',sens.tdof, ...
    'data',def.data(:,1));
dr=fe_def('subdof',dr,fe_c(wire.tdof,.03,'ind'));
